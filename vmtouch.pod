...
Ignores all files and directories that match the provided pattern. The pattern may include wildcards (remember to escape them from your shell). This option stops the crawl, so can be used to ignore directories and all their contents. Can be specified multiple times. Example: vmtouch -i .git -i '*.bak' .
